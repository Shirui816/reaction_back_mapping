...
class Reactions(object):
    def __init__(self, reaction_type, reactants_tuple, reaction_meta):
        r"""Gather reactions and make maps, then choose randomly.
        :param reaction_type: reaction_type, in CG systems, if there were reaction information, <r type> i j k l...
        should be given
        :param reactants_tuple: type of reactants. CG bead type. 1 reaction may correspond to various of monomers
        :param reaction_meta: info of reaction: reaction obj, probability, needed production ids
        """
    ...
    def process(self, reactants_meta):
        r"""The reactants are necessary, the rxn.RunReactants further generate permutations of all reaction sites.
        :param reactants_meta: build class first, processing reactants accordingly.
        :return:
        """
        for idx in self.index:
...
def allowed_p(reacted_atoms, reactions):
    ...
    while True:
        ...
        if idx in seen:
            continue
        seen.add(idx)
        for reaction_map in reaction_maps:  # iterate over all possible permutations generated by rxn.RunReactants
...
class Reactor(object):
    ...
    def process(self, cg_system, reactions):
        ...
        self.meta = nx.Graph()
        global_count = 0
        for node in cg_system.nodes:  # add all molecules first
            atom_idx = {}
        ...
        for r in reactions:
            ...
            key = tuple(sorted(reactant_idx))  # dict key for storing reacted atoms
            reacted_atoms = {}
            ...
            reaction_map, product_idx = allowed_p(reacted_atoms, rxn_tpls)  # get available reaction from all possible
